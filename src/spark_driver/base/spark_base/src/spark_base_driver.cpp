...
#include "spark_base/spark_base_driver.h"
namespace NxSparkBase
{

	SparkBaseDriver::SparkBaseDriver(std::string new_serial_port) : rclcpp::Node("spark_base_node")
	{
		robot_yaw = 0;
		...
		fback_cmd_vel_pub = this->create_publisher<geometry_msgs::msg::Twist>("spark_base/command/velocity", 1);
		odom_reset_sub = this->create_subscription<spark_base::msg::SparkBaseOdom>(
			"spark_base/odom/reset", 1, std::bind(&SparkBaseDriver::resetOdomCb, this, std::placeholders::_1));
		gyro_pub = this->create_publisher<spark_base::msg::GyroMessage>("spark_base/gyro", 5);
		rb_sensor_pub = this->create_publisher<spark_base::msg::SparkBaseSensor>("spark_base/sensor", 5);
		rb_dock_pub = this->create_publisher<spark_base::msg::SparkBaseDock>("spark_base/dock", 5);
		dock_sub = this->create_subscription<std_msgs::msg::String>(
			"spark_base/handle_go2dock", 10, std::bind(&SparkBaseDriver::handleGoDock, this, std::placeholders::_1));
		search_sub = this->create_subscription<std_msgs::msg::String>(
			"spark_base/handle_search_dock", 10, std::bind(&SparkBaseDriver::handleSearchDock, this, std::placeholders::_1));
		...
		stimer = this->create_wall_timer(1s, std::bind(&SparkBaseDriver::checkSerialGoon, this));
	...
	SparkBaseDriver::~SparkBaseDriver()
	{
	}

	void SparkBaseDriver::startComParamInit()
	{
		interface_port_ = std::shared_ptr<SparkBaseInterface>(new SparkBaseInterface(serial_port));
		...
		{
			RCLCPP_INFO(this->get_logger(), "connect to Sparkbase sucessfully.");
		}
		base_receive_thread_ = std::shared_ptr<std::thread>(new std::thread(std::bind(&SparkBaseDriver::process_base_receive_thread, this)));
	}

	void SparkBaseDriver::resetOdomCb(const spark_base::msg::SparkBaseOdom::SharedPtr odom)
	{
		interface_port_->setOdometry(odom->x, odom->y, odom->yaw);
		printf("setting odom!\n");
	}

	void SparkBaseDriver::cmdVelReceived(const geometry_msgs::msg::Twist::SharedPtr cmd_vel)
	{
		interface_port_->drive(cmd_vel->linear.x, cmd_vel->angular.z);
		countSerial++;
	}

	int SparkBaseDriver::pubWheelJointStates(double linear_speed, double angular_speed)
	{
	...
	int SparkBaseDriver::pubGyroMessage(unsigned char *buf, int len)
	{
		spark_base::msg::GyroMessage gyro;
		double now_time = this->get_clock()->now().seconds();
	...
	void SparkBaseDriver::publish_odom()
	{
	...
	void SparkBaseDriver::dealMessageSwitch(unsigned char *recvbuf)
	{
		...
		fb_time[curr_idx] = interface_port_->current_time; // set spark base time which is different from ros time
		...
		spark_base::msg::SparkBaseSensor sensor_msg;
		...
		spark_base::msg::SparkBaseDock dock_msg;
	...
	void SparkBaseDriver::handleGoDock(const std_msgs::msg::String::SharedPtr msg)
	{
	...
	void SparkBaseDriver::handleSearchDock(const std_msgs::msg::String::SharedPtr msg)
	{
	...
	void SparkBaseDriver::hex_printf(unsigned char *buf, int len)
	{
#if 1
		int i;
		for (i = 0; i < len; i++)
	...
	void SparkBaseDriver::process_base_receive_thread()
	{
		...
		while (rclcpp::ok())
		{
			...
			{
				// hex_printf(buf, len);
				getSparkbaseComData(buf, len);
			}
		}
	}

	unsigned char SparkBaseDriver::checkSum(unsigned char *buf)
	{
		unsigned char sum = 0;
		int i;
		for (i = 0; i < buf[2] - 3; i++)
	...
	void SparkBaseDriver::getSparkbaseComData(unsigned char *buf, int len)
	{
		int i;
                    ...
                    printf("\n");
							...
							(recvbuf[framelen - 1] == 0x0A)) // check the header and end
						{
							if (checkSum(recvbuf) == recvbuf[framelen - 3])
							{
								...
								{
									dealMessageSwitch(recvbuf);
								}
							}
							else
							{
								//             RCLCPP_ERROR(this->get_logger(), "sparkbase-check sum error");
#if 0
                            for(j=0;j<framelen; j++)
						...
						{
//            RCLCPP_ERROR(this->get_logger(), "sparkbase-header or ender error error");
#if 0
                        for(j=0; j<framelen; j++)
                            printf(L_RED "%02X " NONE ,(unsigned char)recvbuf[j]);
                        printf("\n");
	...
	void SparkBaseDriver::checkSerialGoon()
	{
		if (countSerial == lastCountSerial)
		{
			interface_port_->drive(0.0, 0.0);
