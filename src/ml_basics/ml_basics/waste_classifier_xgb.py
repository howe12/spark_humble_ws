...
# -*- coding: utf-8 -*-
"""
垃圾分类推理节点 — XGBoost (ROS2)
订阅图像话题，加载 XGBoost 模型进行实时分类。

XGBoost 使用原生 C++ 后端，模型加载方式与 sklearn 不同:
  - 保存: model.save_model('waste_xgboost.json')
  - 加载: model = XGBClassifier(); model.load_model(path)

为避免干扰，发布专属话题 /waste_classification_xgb。
如果 xgboost 未安装，节点启动时会给出明确的安装提示。

用法:
  ros2 run ml_basics waste_classifier_xgb
  ros2 run ml_basics waste_classifier_xgb --ros-args -p image_topic:=/camera/rgb/image_raw
"""
...
# =============================================================================
# WasteClassifierXGBNode
# =============================================================================

class WasteClassifierXGBNode(Node):
    """XGBoost 垃圾分类 ROS2 节点"""

    def __init__(self):
        super().__init__('waste_classifier_xgb')

        self.declare_parameter('image_topic', '/camera/color/image_raw')
        self.declare_parameter('model_file', 'waste_xgboost.json')

        image_topic = self.get_parameter('image_topic').get_parameter_value().string_value
        model_file = self.get_parameter('model_file').get_parameter_value().string_value

        # --- 加载 XGBoost 模型 (原生 API, 非 joblib) ---
        try:
        ...
        self.get_logger().info(f'XGBoost 模型已加载: {model_file}')
        ...
        # --- 专属话题 ---
        self.result_pub = self.create_publisher(String, '/waste_classification_xgb', 1)
        self.confidence_pub = self.create_publisher(Float32, '/waste_classification_xgb/confidence', 1)

        self.image_sub = self.create_subscription(
            Image, image_topic, self.image_callback, 1)

        self.get_logger().info('XGBoost 垃圾分类节点已启动')
        self.get_logger().info(f'订阅话题: {image_topic}')
        self.get_logger().info('发布话题: /waste_classification_xgb')
    ...
    def image_callback(self, msg):
        try:
            ...
            self.get_logger().info(
                f'[XGB] 分类: {self.class_names[int(prediction)]} (置信度: {confidence:.2f})')
        except Exception as e:
            self.get_logger().warn(f'[XGB] 分类失败: {e}')


def main(args=None):
    rclpy.init(args=args)
    node = WasteClassifierXGBNode()
    try:
        rclpy.spin(node)
