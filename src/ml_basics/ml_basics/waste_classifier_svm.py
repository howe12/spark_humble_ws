...
"""
垃圾分类推理节点 — SVM (ROS2)
订阅图像话题，加载 SVM 模型 + StandardScaler 进行实时分类。

SVM 对特征尺度敏感，推理时必须使用训练时的 StandardScaler。

为避免干扰，发布专属话题 /waste_classification_svm。

用法:
  ros2 run ml_basics waste_classifier_svm
  ros2 run ml_basics waste_classifier_svm --ros-args -p image_topic:=/camera/rgb/image_raw
"""
...
# =============================================================================
# WasteClassifierSVMNode
# =============================================================================

class WasteClassifierSVMNode(Node):
    """SVM 垃圾分类 ROS2 节点 (含 StandardScaler)"""

    def __init__(self):
        super().__init__('waste_classifier_svm')

        self.declare_parameter('image_topic', '/camera/color/image_raw')
        self.declare_parameter('model_file', 'waste_svm.pkl')
        self.declare_parameter('scaler_file', 'waste_scaler.pkl')

        image_topic = self.get_parameter('image_topic').get_parameter_value().string_value
        model_file = self.get_parameter('model_file').get_parameter_value().string_value
        scaler_file = self.get_parameter('scaler_file').get_parameter_value().string_value

        # --- 加载 SVM 模型 + StandardScaler ---
        if not os.path.isabs(model_file):
            pkg_dir = os.path.dirname(os.path.abspath(__file__))
            model_file = os.path.join(pkg_dir, '..', 'models', model_file)
            scaler_file = os.path.join(pkg_dir, '..', 'models', scaler_file)

        self.model = joblib.load(model_file)
        self.get_logger().info(f'SVM 模型已加载: {model_file}')

        self.scaler = joblib.load(scaler_file)
        self.get_logger().info(f'StandardScaler 已加载: {scaler_file}')
        ...
        # --- 专属话题 ---
        self.result_pub = self.create_publisher(String, '/waste_classification_svm', 1)
        self.confidence_pub = self.create_publisher(Float32, '/waste_classification_svm/confidence', 1)

        self.image_sub = self.create_subscription(
            Image, image_topic, self.image_callback, 1)

        self.get_logger().info('SVM 垃圾分类节点已启动')
        self.get_logger().info(f'订阅话题: {image_topic}')
        self.get_logger().info('发布话题: /waste_classification_svm')
    ...
    def image_callback(self, msg):
        try:
            ...
            if features is None:
                return
            # SVM 预测前必须用 StandardScaler 标准化
            features_scaled = self.scaler.transform([features])
            prediction = self.model.predict(features_scaled)[0]
            # SVM 的 predict_proba 需要 probability=True 训练时启用
            try:
                confidence = self.model.predict_proba(features_scaled)[0].max()
            except AttributeError:
                confidence = float(self.model.decision_function(features_scaled)[0])
                confidence = 1.0 / (1.0 + np.exp(-confidence))  # sigmoid 近似
            ...
            self.get_logger().info(
                f'[SVM] 分类: {self.class_names[int(prediction)]} (置信度: {confidence:.2f})')
        except Exception as e:
            self.get_logger().warn(f'[SVM] 分类失败: {e}')


def main(args=None):
    rclpy.init(args=args)
    node = WasteClassifierSVMNode()
    try:
        rclpy.spin(node)
