...
class GraspObject(State):
    ...
    def execute(self, userdata):
        ...
        # self.sub.unregister()
        g_node.destroy_subscription(self.sub)
    ...
    def onInit(self):
        ...
        self.sub = g_node.create_subscription(Image, 'camera/color/image_raw', self.image_cb, 10)
        self.pub1 = g_node.create_publisher(Position, 'position_write_topic', 10)
        self.pub2 = g_node.create_publisher(Status, 'pump_topic', 10)
...
class CarryObject():
    ...
    def scene_status_swith_cb(self, req, response):
        '''
    ...
    def timer(self, no, interval):
        ...
        # time.sleep(interval);
        g_node.get_logger().info("req infomaion, status:%s" % (self.switch_status))
        #rospy.loginfo("req infomaion, status:%s" % (self.switch_status))
        if self.switch_status == 1:
            #rospy.loginfo("starting to sm")
            g_node.get_logger().info("starting to sm")

            self.build_state_machine()
            self.execute_sm()
        
        self.switch_status = 0
        g_node.get_logger().info("stopped to sm")
        thread.exit_thread() 
    ...
    def init_service_and_topic(self):
        ...
        #self.cmd_vel_pub = rospy.Publisher('/cmd_vel', Twist, queue_size=1)  
        self.srv = g_node.create_service(Scene, 's_carry_object', self.scene_status_swith_cb)
    ...
    def execute_sm(self):   
        ...
        g_node.get_logger().info('State Machine Outcome: ' + str(sm_outcome))
        intro_server.stop()
    ...
    def shutdown(self):
        ...
        global g_node
        global cmd_vel_pub
        g_node.get_logger().info("Stopping carry object")               
        time.sleep(1)
        cmd_vel_pub.publish(Twist())
        # rospy.spin()
...
def main(args=None):
    ...
    g_node = rclpy.create_node('s_carry_object_node')        
    g_node.get_logger().info("Init carry object")
    cmd_vel_pub = g_node.create_publisher(Twist, 'cmd_vel', 10)
