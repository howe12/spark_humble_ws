...
def msg_callback(data):
	...
	if(data.data == "start"):
		move_position = 0
		collecting = 1
		g_node.get_logger().info('start to collect the hsv!!')
	else:
		collecting = 0
...
def main(args=None):
	...
	g_node = rclpy.create_node('image_converter')
	sub1 = g_node.create_subscription(String, 'start_topic', msg_callback, 10)
	sub2 = g_node.create_subscription(Image, 'camera/color/image_raw', image_callback, 10)
	sub3 = g_node.create_subscription(Status, 'cali_pix_topic', command_callback, 10)
	try:
		rclpy.spin(g_node)
